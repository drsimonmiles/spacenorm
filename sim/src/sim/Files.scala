...
*/
object Files:
  def loadTOML(file: File): Option[Map[String, String]] = {
    ...
    try {
      ...
      case e: Exception =>
        System.err.println(s"Ignoring ${file.getName} as cannot read")
        None
    } finally {
      source.close
    }
  ...
  def loadSettings(file: File): Option[SettingsSpace] = {
    loadTOML(file).map { attributes =>
  ...
  def loadStats(statsFile: File): Option[List[Result]] = {
    val source = Source.fromFile(statsFile)
    try {
      val loaded =
      ...
      case e: Exception =>
        System.err.println(s"Ignoring ${statsFile.getName} as cannot read")
        None
    } finally {
      source.close
    }
